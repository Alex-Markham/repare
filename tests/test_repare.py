...
def test_chain_gaussian_score_matches_manual():
    ...
    score = model.chain_gaussian_score([data])

    centered = data - data.mean(axis=0, keepdims=True)
    cov = centered.T @ centered / n
    Saa = cov[np.ix_([0, 1], [0, 1])]
    inv_Saa = np.linalg.pinv(Saa)
    Scp = cov[np.ix_([2], [0, 1])]
    Spc = Scp.T
    beta_b = Scp @ inv_Saa
    residual_b = (
        cov[np.ix_([2], [2])]
    ...
    sign_a, logdet_a = np.linalg.slogdet(Saa)
    assert sign_a > 0
    sign_b, logdet_b = np.linalg.slogdet(residual_b)
    assert sign_b > 0
    loglik = -0.5 * n * (logdet_a + 2 + logdet_b + 1)
    assert np.isclose(score, -loglik, rtol=1e-5)
