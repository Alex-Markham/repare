...
if edge_idcs.size:
    flips = rng.choice([True, False], size=edge_idcs.size)
    to_neg = edge_idcs[flips]
...
actual_density = float(edge_idcs.size) / float(num_nodes * (num_nodes - 1)) if num_nodes > 1 else 0.0
data_dict = {"obs": obs_dataset} | interv_datasets
np.savez(
    snakemake.output[0],
    weights=weights,
    targets=targets,
    graph_family=np.array("scale_free"),
    actual_density=np.array(actual_density),
    ba_m=np.array(m_param),
    order=order,
    **data_dict,
)
